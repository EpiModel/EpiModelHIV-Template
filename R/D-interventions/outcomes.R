...
# IR100 multiplier
ir100_m <- 100 * year_steps

# Rename raw epi trackers to standardized outcome names
mutate_outcomes <- function(d) {
  d |>
    mutate(
      lst_ir100_B = hiv.incid.B / (num.B - hiv.inf.B - hiv.incid.B) * ir100_m,
      lst_ir100_H = hiv.incid.H / (num.H - hiv.inf.H - hiv.incid.H) * ir100_m,
      lst_ir100_W = hiv.incid.W / (num.W - hiv.inf.W - hiv.incid.W) * ir100_m,
      cml_incid_B = hiv.incid.B,
      cml_incid_H = hiv.incid.H,
      cml_incid_W = hiv.incid.W
    )
}
