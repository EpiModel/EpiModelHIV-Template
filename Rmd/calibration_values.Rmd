...
## ir100.hiv.dx - section 8.2

```{r}
p <- calib_plot_infos[["ir100.hiv.dx"]]
make_calib_plot(d_outs, p)
```
...
col_names <- paste0(c("Black", "Hispanic", "White/Other"), "MSM")
row_names <- c(
  "Target Statistic: HIV Diagnosed Incidence per 100 MSM",
  "Simulations: HIV IR100 (med [IQR])",
  "Transmission Scaler (per act)"
)

fmtr <- scales::label_percent(0.1)
tar_names <- paste0("ir100.hiv.dx.", races_names)
tar <- targets[tar_names] |> fmtr()
p_names <- paste0("hiv.trans.scale")
prms <- param[[p_names]] |> scales::label_number(0.01)()
...
row_names <- c(
  ...
  "Calibrated Transmission Probability (per act)"
)
